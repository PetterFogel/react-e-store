...
export default class UserProvider extends Component<any, State> {
  state: State = {
    shippingPriceState: 0,
    user: {
      name: "",
  ...
  addDeliveryToState = (
    ...
    tomorrow.setDate(
      today.getDate() + (Math.floor(Math.random() * randomDay) + addNumber)
    );
          ...
          if (this.state.user.email !== "") {
            if (this.state.user.email.includes("@")) {
              ...
              if (this.state.user.zip !== "") {
                if (
                  this.state.user.zip.length < 5 ||
                  this.state.user.zip.length >= 6
                ) {
                  event.preventDefault();
                } else {
                  event.preventDefault();
